    ...
    <FigureProvider>
      <div className="Page blog-post">
        ...
        <Figure label="three-problems" caption={<>Three problems ignored by many recent generative AI models for biomolecule design. (left) Experimental Data: Models often only use the protein target structure (top), ignoring extremely useful experimental data on known binders that could also be used to inform binder design (bottom). (middle) Therapeutic Modality: Models often only generate proteins and natural peptides (top) and not small molecules (bottom) due to the relative ease of synthesizing and experimentally validating proteins and natural peptides, despite the many advantages of small molecules like an oral route of administration and cost of goods. (right) Druglike Properties: Models often exclusively focus on binder potency (top), ignoring the wide range of properties beyond potency that are necessary in a drug (bottom). (PDB structures <a href="https://www.rcsb.org/structure/1X70" target="_blank">1X70</a> and <a href="https://www.rcsb.org/structure/8SK7" target="_blank">8SK7</a> visualized with PyMol.)</>}>
          <img src="/images/blog/synthemol_merck/overview/three-problems.webp" alt="Three problems ignored by many recent generative AI models for biomolecule design." />
        </Figure>
        ...
        <Figure label="synthemol" caption={<>SyntheMol is a generative AI method for synthesizable small molecule design that uses either a Monte Carlo tree search (SyntheMol-MCTS) or reinforcement learning (SyntheMol-RL). SyntheMol exclusively generates molecules within a combinatorial chemical space composed of molecular building blocks and chemical reactions, and it uses trained molecular property predictors to score generated molecules to find the best drug candidates. These molecules can then be easily synthesized and experimentally validated. (Figure reproduced from the original <a href="https://www.nature.com/articles/s42256-024-00809-7" target="_blank">SyntheMol paper</a>.)</>}>
          <img src="/images/blog/synthemol_merck/overview/synthemol.webp" alt="SyntheMol is a generative AI method for synthesizable small molecule design." />
        </Figure>
        ...
        <Figure label="synthemol-rl" caption={<>Experimental results of molecules generated by SyntheMol-RL. (top) SyntheMol-RL (specifically, two variants called RL-Chemprop and RL-MLP) generated more molecules capable of inhibiting the growth of <span style={{ fontStyle: "italic" }}>S. aureus</span> than other generative (MCTS) and virtual screening (VS-Chemprop) methods. (bottom left) The most effective molecule in the in vitro growth inhibition experiments, which we called synthecin, was generated by SyntheMol-RL. (bottom right) Synthecin effectively inhibited the growth of methicillin-resistance <span style={{ fontStyle: "italic" }}>S. aureus</span> in a mouse skin infection model. (Figure reproduced from the <a href="https://www.biorxiv.org/content/10.1101/2025.05.17.654017v1" target="_blank">SyntheMol-RL paper</a>.)</>}>
          <img src="/images/blog/synthemol_merck/overview/synthemol-rl.webp" alt="SyntheMol-RL designed effective antibiotic candidates for Staphylococcus aureus." />
        </Figure>
        ...
        <Figure label="human-potency" cols={2} caption={<>Distribution of the biochemical potency of ~8k small molecules against the target. (left) The full distribution of potency values detected in the assay. (right) The distribution of potency values less than 200 nM, with three potency thresholds from strong (25 nM) to weak (100 nM).</>}>
          <img src="/images/blog/synthemol_merck/human/potency-full.webp" alt="Full distribution of the biochemical potency of ~8k small molecules against the target." />
          <img src="/images/blog/synthemol_merck/human/potency-zoom.webp" alt="Distribution of the biochemical potency of ~8k small molecules against the target less than 200 nM, with three potency thresholds from strong (25 nM) to weak (100 nM)." />
        </Figure>
        ...
        <Figure label="human-lbdd-sbdd" caption={<>(left) Ligand-based drug design (LBDD), in which ML models are trained on experimental binding affinity data of small molecules tested against the target protein. These models are then applied to score potential small molecule drug candidates based on the small molecule structure alone (no protein structure). LBDD can be done with alternative methods like <a href="https://www.eyesopen.com/rocs" target="_blank">ROCS</a>, which uses ligand shape similarity, but here we focus on ML-based methods. (right) Structure-based drug design (SBDD), in which a model (either physics or ML) uses the 3D structure of the protein target to predict small molecule binding to the protein. Here, PDB structure <a href="https://www.rcsb.org/structure/1X70" target="_blank">1X70</a> is visualized with PyMol. Molecules shown are for illustration purposes only and are taken for unrelated targets in the literature.</>}>
          <img src="/images/blog/synthemol_merck/human/lbdd-sbdd.webp" alt="Ligand-based drug design (LBDD) and structure-based drug design (SBDD)." />
        </Figure>
        ...
        <Figure label="docking-scores" cols={2} caption={<>Distribution of docking scores for ~1M molecules docked against the target. Docking scores &lt; -100 were set to -100 and docking scores &gt; 0 were set to 0. (left) The full distribution of docking scores. (right) The distribution of docking scores less than 0, with three docking score thresholds from strong (-50) to weak (-30).</>}>
          <img src="/images/blog/synthemol_merck/human/docking-full.webp" alt="Full distribution of docking scores for ~1M molecules docked against the target." />
          <img src="/images/blog/synthemol_merck/human/docking-zoom.webp" alt="Distribution of docking scores less than 0, with three docking score thresholds from strong (-50) to weak (-30)." />
        </Figure>
        ...
        <Figure label="human-chemprop-performance" cols={2} caption={<>Performance of Chemprop-RDKit property predictors trained to predict (left) log-transformed experimentally determined on-target potency or (right) on-target docking score for the target.</>}>
            <img src="/images/blog/synthemol_merck/human/chemprop-potency.webp" alt="Performance of Chemprop-RDKit property predictors trained to predict log-transformed experimentally determined on-target potency for the target." />
          <img src="/images/blog/synthemol_merck/human/chemprop-docking.webp" alt="Performance of Chemprop-RDKit property predictors trained to predict on-target docking score for the target." />
        </Figure>
        ...
        <Figure label="human-weighting" cols={2} caption={<>The SyntheMol-RL dynamic weighting mechanism balances the two on-target potency objectives &ndash; experimental potency and docking score &ndash; across rollouts (iterations of generation). (left) The rolling average success rate of generated molecules according to both objectives, where experimental success is potency &lt; 100 nM and docking success is docking score &gt; -50. (right) The weight of the experimental potency and docking score objectives set by SyntheMol-RL across rollouts. Note that although the success rates are similar, SyntheMol-RL selects different weights since the two objectives are on different scales.</>}>
          <img src="/images/blog/synthemol_merck/human/success.webp" alt="The rolling average success rate of generated molecules according to both objectives, where experimental success is potency &lt; 100 nM and docking success is docking score &gt; -50." />
          <img src="/images/blog/synthemol_merck/human/weights.webp" alt="The weight of the experimental potency and docking score objectives set by SyntheMol-RL across rollouts." />
        </Figure>
        ...
        <Figure label="human-vs-real" cols={2} caption={<>(left) The distribution of predicted log on-target potency values for molecules generated by SyntheMol-RL (blue) and molecules randomly sampled from the Enamine REAL Space (orange). (right) The distribution of predicted docking scores for molecules generated by SyntheMol-RL (blue) and molecules randomly sampled from the Enamine REAL Space (orange).</>}>
          <img src="/images/blog/synthemol_merck/human/generated-vs-real-potency.webp" alt="The distribution of predicted log on-target potency values for molecules generated by SyntheMol-RL (blue) and molecules randomly sampled from the Enamine REAL Space (orange)." />
          <img src="/images/blog/synthemol_merck/human/generated-vs-real-docking.webp" alt="The distribution of predicted docking scores for molecules generated by SyntheMol-RL (blue) and molecules randomly sampled from the Enamine REAL Space (orange)." />
        </Figure>
        ...
        <Figure label="human-potency-vs-docking" caption={<>The distribution of predicted experimental potency and predicted docking scores for molecules generated by SyntheMol-RL. The lower right quadrant contains 178 molecules that satisfy both objectives simultaneously (predicted experimental potency &lt; 100 nM and predicted docking score &lt; -40).</>}>
          <img src="/images/blog/synthemol_merck/human/potency-vs-docking.webp" alt="The distribution of predicted experimental potency and predicted docking scores for molecules generated by SyntheMol-RL." />
        </Figure>
        ...
        <p>Finally, after months of anxious anticipation, we received experimental results, and we were excited to see that four of the compounds were potent, with IC<sub>50</sub> &lt; 10uM (potencies of 3.1 uM, 6.1 uM, 6.3 uM, and 7.6 uM). Although the potencies were weaker than we had hoped, these compounds all had novel structures by design, so can potentially address the possible ADMET issues identified in the existing chemical series. Our <a href="https://pubs.acs.org/doi/full/10.1021/acs.jcim.5c00538" target="_blank">Merck global predictive ADMET models</a> suggest that all four hits have good P<sub>app</sub> (&gt;15 x 10-6 cm/s), low P-gp efflux (ratio &lt;2.5), and low hERG channel binding (IC<sub>50</sub> in hERG binding assay &gt;20uM), but display moderate solubility (&lt;50uM in water and simulated intestinal fluid) and short rat MRTs (0.5-1.5 hrs). Our next steps are to attempt a co-crystal structure of the molecules and experimentally assess their ADMET profile.</p>
        ...
        <Figure label="antibiotic-potency" caption={<>The distribution of log-transformed experimental potency values of 2.4k molecules for the target of interest in each of five bacterial species. (diagonal) Histogram of the log potency values of molecules tested against a single species. (off-diagonal) Scatter plot showing the log potency values of molecules tested against both of each pair of species.</>}>
          <img src="/images/blog/synthemol_merck/antibiotic/potency.webp" alt="The distribution of log-transformed experimental potency values of 2.4k molecules for the target of interest in each of five bacterial species." />
        </Figure>

        <p>As with the first program, Merck had already discovered numerous potent molecules targeting the desired protein, but the majority exhibit potential ADMET liabilities. One of the key issues is that these molecules can potentially bind the human hERG channel, which may be a cardiovascular risk. Therefore, like the first program, we need to identify molecules with new chemical structures that retain potency while reducing the measured hERG binding. Despite the similarities, this target differs in two major ways.</p>
        ...
        <Figure label="antibiotic-lhs-lnk-rhs" caption={<>The overall linear structure of most inhibitors, which take the form of left-hand side (LHS), linker (LNK), and right-hand side (RHS). Note that the example molecule shown is taken from a different publicly-known program and is not related to this target.</>}>
          <img src="/images/blog/synthemol_merck/antibiotic/lhs-lnk-rhs.webp" alt="The overall linear structure of most inhibitors, which take the form of left-hand side (LHS), linker (LNK), and right-hand side (RHS)." />
        </Figure>
        ...
        <Figure label="antibiotic-chemprop-performance" cols={3} caption={<>Performance of 10 Chemprop-RDKit property predictors trained to predict log-transformed experimental potency across five bacterial species.</>}>
          <img src="/images/blog/synthemol_merck/antibiotic/chemprop-acinetobacter.webp" alt="Performance of 10 Chemprop-RDKit property predictors trained to predict log-transformed experimental potency on A. baumannii." />
          <img src="/images/blog/synthemol_merck/antibiotic/chemprop-escherichia.webp" alt="Performance of 10 Chemprop-RDKit property predictors trained to predict log-transformed experimental potency on E. coli." />
          <img src="/images/blog/synthemol_merck/antibiotic/chemprop-klebsiella.webp" alt="Performance of 10 Chemprop-RDKit property predictors trained to predict log-transformed experimental potency on K. pneumoniae." />
          <img src="/images/blog/synthemol_merck/antibiotic/chemprop-pseudomonas.webp" alt="Performance of 10 Chemprop-RDKit property predictors trained to predict log-transformed experimental potency on P. aeruginosa." />
          <img src="/images/blog/synthemol_merck/antibiotic/chemprop-staphylococcus.webp" alt="Performance of 10 Chemprop-RDKit property predictors trained to predict log-transformed experimental potency on S. aureus." />
        </Figure>
        ...
        <Figure label="antibiotic-herg" cols={2} caption={<>(left) Distribution of hERG potency values across an internal set of 370k molecules at Merck. (right) Performance of Chemprop-RDKit property predictors trained to predict log-transformed hERG potency.</>}>
          <img src="/images/blog/synthemol_merck/antibiotic/herg.webp" alt="Distribution of hERG potency values across an internal set of 370k molecules at Merck." />
          <img src="/images/blog/synthemol_merck/antibiotic/chemprop-herg.webp" alt="Performance of Chemprop-RDKit property predictors trained to predict log-transformed hERG potency." />
        </Figure>
        ...
        <Figure label="antibiotic-weighting" cols={2} caption={<>The success rate (left) and weights (right) of SyntheMol-RL when generating molecules with a six-parameter optimization for five bacterial potency values and one hERG potency value.</>}>
          <img src="/images/blog/synthemol_merck/antibiotic/success.webp" alt="The success rate of SyntheMol-RL when generating molecules with a six-parameter optimization for five bacterial potency values and one hERG potency value." />
          <img src="/images/blog/synthemol_merck/antibiotic/weights.webp" alt="The weights of the six objectives set by SyntheMol-RL when generating molecules with a six-parameter optimization for five bacterial potency values and one hERG potency value." />
        </Figure>
        ...
        <Figure label="antibiotic-vs-real" cols={3} caption={<>(all except bottom right) The distribution of predicted log-transformed bacterial potency values for molecules generated by SyntheMol-RL (blue) and molecules randomly sampled from the Enamine REAL Space (orange). (bottom right) The distribution of predicted log-transformed hERG potency values for molecules generated by SyntheMol-RL (blue) and molecules randomly sampled from the Enamine REAL Space (orange).</>}>
          <img src="/images/blog/synthemol_merck/antibiotic/generated-vs-real-acinetobacter.webp" alt="The distribution of predicted log-transformed bacterial potency values for molecules generated by SyntheMol-RL (blue) and molecules randomly sampled from the Enamine REAL Space (orange) for A. baumannii." />
          <img src="/images/blog/synthemol_merck/antibiotic/generated-vs-real-escherichia.webp" alt="The distribution of predicted log-transformed bacterial potency values for molecules generated by SyntheMol-RL (blue) and molecules randomly sampled from the Enamine REAL Space (orange) for E. coli." />
          <img src="/images/blog/synthemol_merck/antibiotic/generated-vs-real-klebsiella.webp" alt="The distribution of predicted log-transformed bacterial potency values for molecules generated by SyntheMol-RL (blue) and molecules randomly sampled from the Enamine REAL Space (orange) for K. pneumoniae." />
          <img src="/images/blog/synthemol_merck/antibiotic/generated-vs-real-pseudomonas.webp" alt="The distribution of predicted log-transformed bacterial potency values for molecules generated by SyntheMol-RL (blue) and molecules randomly sampled from the Enamine REAL Space (orange) for P. aeruginosa." />
          <img src="/images/blog/synthemol_merck/antibiotic/generated-vs-real-staphylococcus.webp" alt="The distribution of predicted log-transformed bacterial potency values for molecules generated by SyntheMol-RL (blue) and molecules randomly sampled from the Enamine REAL Space (orange) for S. aureus." />
          <img src="/images/blog/synthemol_merck/antibiotic/generated-vs-real-herg.webp" alt="The distribution of predicted log-transformed hERG potency values for molecules generated by SyntheMol-RL (blue) and molecules randomly sampled from the Enamine REAL Space (orange)." />
        </Figure>
        ...
        <Figure label="antibiotic-potency-vs-herg" cols={3} caption={<>Distributions of predicted log-transformed bacterial potency and predicted log-transformed hERG potency for molecules generated by SyntheMol-RL for each of the five bacterial species. The lower right quadrant shows the number of molecules that satisfy both objectives simultaneously (predicted experimental potency &lt; 2 µg/mL and predicted hERG potency &gt; 30 nM).</>}>
          <img src="/images/blog/synthemol_merck/antibiotic/potency-vs-herg-acinetobacter.webp" alt="Distributions of predicted log-transformed bacterial potency and predicted log-transformed hERG potency for molecules generated by SyntheMol-RL for A. baumannii." />
          <img src="/images/blog/synthemol_merck/antibiotic/potency-vs-herg-escherichia.webp" alt="Distributions of predicted log-transformed bacterial potency and predicted log-transformed hERG potency for molecules generated by SyntheMol-RL for E. coli." />
          <img src="/images/blog/synthemol_merck/antibiotic/potency-vs-herg-klebsiella.webp" alt="Distributions of predicted log-transformed bacterial potency and predicted log-transformed hERG potency for molecules generated by SyntheMol-RL for K. pneumoniae." />
          <img src="/images/blog/synthemol_merck/antibiotic/potency-vs-herg-pseudomonas.webp" alt="Distributions of predicted log-transformed bacterial potency and predicted log-transformed hERG potency for molecules generated by SyntheMol-RL for P. aeruginosa." />
          <img src="/images/blog/synthemol_merck/antibiotic/potency-vs-herg-staphylococcus.webp" alt="Distributions of predicted log-transformed bacterial potency and predicted log-transformed hERG potency for molecules generated by SyntheMol-RL for S. aureus." />
        </Figure>
